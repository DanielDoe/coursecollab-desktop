...
export default function GuestRecommendationAccountRequestsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await instructorApiFetch("/api/instructor/recommendation-guest-account-requests", {
        headers: buildAdminAuthorizedApiHeaders({ "x-instructor-id": String(inst.id) }),
      })
  ...
  const totals = useMemo(() => {
    ...
    const lettersCourse = rows.reduce((acc, r) => acc + (Number(r.rec_letter_total) || 0), 0)
    return { pending, approved, total: rows.length, lettersCourse }
  }, [rows])
  ...
  const approve = async (id: number) => {
    ...
    try {
      const res = await instructorApiFetch(`/api/instructor/account-requests/${id}/approve`, {
        method: "POST",
        headers: buildAdminAuthorizedApiHeaders({
    ...
    const actions = (
      <div className="flex w-full shrink-0 flex-row gap-2">
        <Button
          type="button"
          size="sm"
          className="h-9 min-h-0 flex-1 rounded-lg px-3 text-xs sm:max-h-9 sm:text-sm"
          disabled={!pending || b}
          onClick={() => void approve(r.id)}
        >
          {b ? <Loader2 className="h-4 w-4 shrink-0 animate-spin" aria-hidden /> : "Approve"}
        </Button>
        <Button
          type="button"
          size="sm"
          variant="outline"
          className="h-9 min-h-0 flex-1 rounded-lg border-red-200 px-3 text-xs text-red-700 hover:bg-red-50 dark:border-red-900 dark:text-red-400 sm:text-sm"
          disabled={!pending || b}
          onClick={() => {
            setRejectId(r.id)
            setRejectReason("")
        ...
        <CardWrapper delay={0} hover={false} className="min-w-0 max-w-full overflow-hidden">
          <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-start sm:justify-between lg:p-5">
            ...
            <div className="flex w-full min-w-0 flex-col gap-3 border-t border-slate-100 pt-4 dark:border-slate-800 sm:w-auto sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0 md:max-w-[12rem]">
              {actions}
              <Link
                href="/admin/dashboard-v2/recommendations/pending"
      ...
      <div className="flex flex-col gap-3 p-4 sm:p-5">
        <div className="relative min-w-[12rem]">
          ...
          <Input
            ...
            placeholder="Search name, email, organization, rejection reason…"
            className="h-10 rounded-xl border-slate-200 bg-white pl-9 text-sm dark:border-slate-700 dark:bg-slate-950"
            aria-label="Search Career Member applicants"
          />
        </div>
        <div className="grid grid-cols-1 gap-2 min-[560px]:grid-cols-[1fr_auto] min-[560px]:items-center">
          <div className="flex min-w-0 flex-wrap gap-2">
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value as StatusFilter)}
              className={cn(
                "h-10 min-h-10 w-full min-w-0 max-w-none rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 sm:min-w-[10rem] sm:max-w-[14rem] sm:flex-initial dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200",
              )}
            ...
            <select
              ...
              className={cn(
                "h-10 min-h-10 w-full min-w-0 max-w-none rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 sm:min-w-[12rem] sm:max-w-[18rem] sm:flex-initial dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200",
              )}
              aria-label="Sort"
            >
              <option value="submitted_desc">Newest application</option>
              <option value="submitted_asc">Oldest application</option>
              <option value="letters_desc">Most letters in course</option>
              <option value="recent_activity_desc">Recent letter activity</option>
              <option value="name_asc">Applicant A–Z</option>
            </select>
          </div>
          <div className="flex w-full min-w-0 justify-end min-[560px]:w-auto min-[560px]:shrink-0">
            <Button
              type="button"
        ...
        <p className="text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
          Letter metrics are for <strong className="font-medium text-slate-600 dark:text-slate-300">your account</strong> and the{" "}
          <strong className="font-medium text-slate-600 dark:text-slate-300">currently selected course</strong> only—switch course in the
          top bar to see a different slice.
        </p>
      </div>
    </CardWrapper>
  )
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="w-full min-w-0 max-w-full space-y-5 overflow-x-hidden pb-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex gap-4 min-w-0">
          <div
            className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-emerald-500/25 bg-gradient-to-br from-emerald-500/[0.12] to-emerald-600/[0.06] shadow-sm dark:border-emerald-400/25 dark:from-emerald-400/10"
            aria-hidden
          >
            <UserPlus className="h-6 w-6 text-emerald-700 dark:text-emerald-400" />
          </div>
          <div className="min-w-0 space-y-1">
            <h1 className="text-balance text-lg font-bold tracking-tight text-slate-900 dark:text-white sm:text-2xl">
              Career Member requests · recommendation letters
            </h1>
            <p className="text-sm text-slate-600 dark:text-slate-400 max-w-2xl leading-relaxed">
              External applicants who selected <strong className="font-medium text-slate-700 dark:text-slate-300">recommendation letter</strong>{" "}
              access. Use search and filters to triage; each card shows how many letters they already have on-file with you in this course.
            </p>
            {!busy && rows.length > 0 ? (
              <div className="flex flex-wrap gap-2 pt-2">
                <Badge variant="secondary" className="rounded-md font-normal">
                  {totals.total} applicants
                </Badge>
                <Badge variant="secondary" className="rounded-md border-amber-500/30 bg-amber-500/10 font-normal text-amber-950 dark:text-amber-100">
                  {totals.pending} pending
                </Badge>
                <Badge variant="secondary" className="rounded-md border-emerald-500/30 bg-emerald-500/10 font-normal text-emerald-950 dark:text-emerald-100">
                  {totals.approved} approved
                </Badge>
                <Badge variant="secondary" className="rounded-md font-normal">
                  {totals.lettersCourse} total letter rows (course)
                </Badge>
              </div>
            ) : null}
          </div>
      ...
      {busy ? (
        ...
        <CardWrapper hover={false} delay={0} className="p-10 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
            When someone applies on the Career Member page and selects recommendation letter access, pending rows show up here.
          </p>
        </CardWrapper>
      ) : filtered.length === 0 ? (
