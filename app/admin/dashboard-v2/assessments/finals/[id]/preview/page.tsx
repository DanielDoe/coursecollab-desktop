...
interface PreviewFinalPageProps {
  params: Promise<{ id: string }>
}

export default function PreviewFinalPage({ params }: PreviewFinalPageProps) {
  const { id } = use(params)
      ...
      <EmbedModuleCard className="border-0 shadow-none">
        <div className="w-full min-w-0 overflow-x-hidden p-3 sm:p-4 md:p-5">
          <AssessmentProvider type="final">
            <QuizPreview quizId={id} assessmentType="final" embedInDashboard />
          </AssessmentProvider>
        </div>
      </EmbedModuleCard>
    </PageEnter>
