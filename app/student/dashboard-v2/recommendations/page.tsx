...
export default function StudentDashboardRecommendationsPage() {
  ...
  if (!studentKey && !err) {
    return (
      <div className="flex justify-center py-20">
        <Loader2 className="h-8 w-8 animate-spin text-[var(--cc-accent-dark)]" aria-hidden />
      </div>
    )
  }
  ...
  return (
    ...
    >
      {err && (
        <p
          className="text-sm text-red-600 dark:text-red-400 rounded-lg border border-red-200/80 dark:border-red-900/40 bg-red-50/90 dark:bg-red-950/25 px-3 py-2.5"
          role="alert"
        >
          {err}
        </p>
      ...
      <EmbedModuleCard
        toolbar={
          <EmbedToolbar>
            ...
            />
            <EmbedToolbarActions>
              <span className="hidden sm:inline text-xs text-[var(--cc-text-muted)] tabular-nums px-2">
      ...
      >
        <div className="grid gap-5 p-4 pt-0 sm:p-5 sm:pt-0 xl:grid-cols-2 xl:items-start">
