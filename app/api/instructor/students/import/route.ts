  ...
  try {
    ...
    if (session_id) {
      const sessionResult = await sql`
        ...
        WHERE id = ${session_id} AND course_id = ${scope.course.id}
      `
      ...
      const resolved = await resolveSessionRowByCode(String(session_code), scope.course.id)
      if (!resolved) {
      ...
      const resolved = await resolveSessionRowByCode(String(section), scope.course.id)
      if (!resolved) {
