...
import { resolveStudentCoraPackCheckout } from "@/lib/cora/credits/mobile-payment"
import {
...
/** Create PaymentIntent after PaymentSheet confirms a student Cora Credit Pack. */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!Number.isFinite(studentId) || !packId || !paymentMethodId) {
      return NextResponse.json(
        { error: "studentId, packId, and paymentMethodId are required" },
        { status: 400 },
      )
    }
    ...
    console.error("[cora-credits/mobile-payment-intent] student failed:", error)
    return NextResponse.json(
      {
