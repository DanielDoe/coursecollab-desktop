...
export async function POST(
  ...
  try {
    ...
    if (!deck || !studentCanEditDeck(deck, auth.studentDbId)) {
      return NextResponse.json({ error: "Deck not found or not editable" }, { status: 403 })
    }
    ...
    console.error("[student/flashcards/decks/[deckId]/cards POST]", error)
