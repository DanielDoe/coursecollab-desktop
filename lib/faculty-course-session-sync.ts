...
function applyOfferingToSession(next: SessionRecord, offering: FacultyCourseOption): void {
  const previousCourseId =
    next.selectedCourseId != null && String(next.selectedCourseId).trim() !== ""
      ? Number(next.selectedCourseId)
      : null
  const previousSessionId = sessionScopeId(next)
  const previousSessionCode =
    typeof next.selectedSessionCode === "string" ? next.selectedSessionCode.trim() : ""
  ...
  if (offering.session_id != null) {
    ...
    previousCourseId === offering.course_id
  ) {
    next.selectedSessionId = previousSessionId
    if (previousSessionCode) next.selectedSessionCode = previousSessionCode
  } else {
    delete next.selectedSessionId
    delete next.selectedSessionCode
  }
...
export function offeringKeyFor(o: FacultyCourseOption): string {
  return facultyOfferingKey(o.course_id, o.academic_term_id, o.session_id ?? null)
}

export function findOfferingMatch(
  ...
  if (sessionId != null) {
    const exactSession = courses.find(
      (c) =>
        ...
        (termId == null || (c.academic_term_id ?? null) === termId) &&
        c.session_id === sessionId,
    )
    if (exactSession) return exactSession

    const bySessionSameCourse = courses.find(
      (c) => c.course_id === courseId && c.session_id === sessionId,
    )
    if (bySessionSameCourse) return bySessionSameCourse

    // Keep the selected lab section. A course+term row without session_id would
    // blank the switcher and drop x-session-id so students never see the live class.
    return undefined
  }
  ...
  if (termId != null) {
    const exact = courses.find(
      (c) =>
        c.course_id === courseId &&
        c.academic_term_id === termId &&
        (c.session_id == null || c.session_id === undefined),
    )
    if (exact) return exact

    const sameTerm = courses.filter((c) => c.course_id === courseId && c.academic_term_id === termId)
    if (sameTerm.length === 1) return sameTerm[0]
  }
...
export function reconcileFacultySelectedCourse(
  ...
  const hasSelectedCourse = Number.isFinite(selectedNum)

  if (courses.length === 0) {
    // Empty list is a fetch miss, not "this instructor has no courses".
...
/** Map stored scope onto a Select item key that actually exists in the loaded offerings. */
export function resolveFacultyCourseSelectValue(
  session: SessionRecord,
  offerings: FacultyCourseOption[],
): string {
  const stored = facultyCourseSelectValue(session)
  if (!stored) return ""
  const exact = offerings.find((o) => offeringKeyFor(o) === stored)
  if (exact) return offeringKeyFor(exact)

  const courseId = Number(session.selectedCourseId)
  const match = findOfferingMatch(offerings, courseId, sessionTermId(session), sessionScopeId(session))
  if (match) return offeringKeyFor(match)
  return stored
}
