...
export function ProgressReviewDashboardV2() {
  ...
  const metaLine =
    review?.sections && review.progressData?.gradebook
      ? `${review.progressData.gradebook.totalScore.toFixed(1)}% overall · ${periodBadgeLabel(
          (review.reviewPeriod || "midterm") as ProgressReviewPeriod,
          review.asOfDate,
        )}`
      : "Personalized feedback from your instructor"

  const body = !review?.sections ? (
    ...
    <ProgressReviewContent review={review} />
  )

  return (
    <StudentModuleHubLayout
      moduleId="progress-review"
      title="Progress Review"
      metaLine={metaLine}
      metaSuffix="grades, strengths, and your action plan"
      hideSideMenu
...
function ProgressReviewContent({ review }: { review: ReviewPayload }) {
  const s = review.sections!
  const gb = review.progressData?.gradebook
  const periodLabel = periodBadgeLabel(
    (review.reviewPeriod || "midterm") as ProgressReviewPeriod,
    review.asOfDate,
  )

  return (
    <div className="space-y-5 sm:space-y-6 w-full min-w-0 pb-4">
