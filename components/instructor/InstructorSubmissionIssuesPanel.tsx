...
import { AM_PANEL_SCROLL } from "@/lib/assessments/assessment-management-surface-classes"
...
export function InstructorSubmissionIssuesPanel({ embedInAdmin }: { embedInAdmin?: boolean } = {}) {
  ...
  return (
    <div className={cn("flex w-full min-w-0 flex-col", embedInAdmin && "min-h-0 flex-1")}>
      <InstructorPolicySurfaceCard className="w-full" fillHeight={embedInAdmin}>
        <div className={cn(embedInAdmin ? "flex min-h-0 flex-1 flex-col gap-4" : "space-y-4")}>
          <div className={cn(embedInAdmin && "shrink-0 space-y-3")}>
          ...
          <div className={cn(embedInAdmin ? cn(AM_PANEL_SCROLL, "space-y-4") : "space-y-4")}>{diagnosticsBody}</div>
        </div>
